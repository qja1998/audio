...
class Model(nn.Module):
    def __init__(self, in_channels=4,
        ...
        self.encoder_bot = Encoder(in_channels, num_hiddens, num_residual_layers, num_residual_hiddens, stride=8)#4
        ...
        self.decoder = Decoder(embedding_dim + embedding_dim, in_channels, num_hiddens, num_residual_layers, num_residual_hiddens, stride=8)#4
    ...
    def encode(self, x):
        ...
        x = x.reshape(-1, 1, 1024*80)
        x = self.conv1d(x).reshape(-1, 4, 80, 256)
    ...
    def decode(self, quantized_t, quantized_b):
        ...
        dec = self.decoder(quantized).reshape(-1, 4, 256*80)
        dec = self.convTrans1d(dec).reshape(-1, 1, 80, 1024)
    ...
    def forward(self, x):
        quantized_t, quantized_b, loss, preplexity, id_t, id_b = self.encode(x)
