...
class Model(nn.Module):
    def __init__(self, in_channels=4,
        ...
        self.conv1d = nn.Sequential(nn.Conv1d(1, 2, kernel_size=4, stride=2, padding=1),
                                    nn.Conv1d(2, 4, kernel_size=4, stride=2, padding=1)
                                    )
        self.encoder_bot = Encoder(in_channels, num_hiddens, num_residual_layers, num_residual_hiddens, stride=2)#4
        self.pre_vq_conv_bot = nn.Conv2d(in_channels=num_hiddens,
                                     out_channels=embedding_dim,
                                     kernel_size=1, stride=1)
        self.encoder_mid = Encoder(num_hiddens, num_hiddens, num_residual_layers, num_residual_hiddens, stride=2)
        self.pre_vq_conv_mid = nn.Conv2d(in_channels=num_hiddens + embedding_dim,
                                     out_channels=embedding_dim,
                                     kernel_size=1, stride=1)
        ...
        self.vq_mid = VectorQuantizer(embedding_dim, num_embeddings, commitment_cost)
        self.decoder_mid = Decoder(embedding_dim, embedding_dim, num_hiddens, num_residual_layers, num_residual_hiddens, stride=2)
        ...
        self.upsample_tx2 = nn.ConvTranspose2d(
            embedding_dim, embedding_dim, 6, stride=4, padding=1
        )
        self.decoder = Decoder(embedding_dim + embedding_dim + embedding_dim, in_channels, num_hiddens, num_residual_layers, num_residual_hiddens, stride=2)#4 <- 8
        self.convTrans1d = nn.Sequential(nn.ConvTranspose1d(4, 2, kernel_size=4, stride=2, padding=1),
    ...
    def encode(self, x):
        ...
        x = x.reshape(-1, 1, 512*80) # 1024*80
        x = self.conv1d(x).reshape(-1, 4, 80, 128) # 4, 80, 256
        enc_b = self.encoder_bot(x)         # 1 -> 128
        enc_m = self.encoder_mid(enc_b)
        enc_t = self.encoder_top(enc_m)     # 128 -> 128
        ...
        enc_m = self.pre_vq_conv_mid(enc_m)
        diff_m, quantized_m, perplexity_m, id_m = self.vq_mid(enc_m) # 64 -> 64
        diff_m = diff_m.unsqueeze(0)
        dec_m = self.decoder_mid(quantized_m) # 64 -> 64
        #print(dec_t.shape, enc_b.shape)
        
        enc_b = torch.cat([dec_m, enc_b], 1)  # 64 + 128
        enc_b = self.pre_vq_conv_top(enc_b)   # 64 + 128 -> 64
        diff_b, quantized_b, perplexity_b, id_b = self.vq_bot(enc_b) # 64 -> 64
        diff_b = diff_b.unsqueeze(0)

        return (quantized_t, quantized_m, quantized_b), torch.mean(diff_t + diff_b + diff_m), torch.mean(perplexity_t + perplexity_b + perplexity_m), (id_t, id_m, id_b)

    def decode(self, quantized_t, quantized_m, quantized_b):
        #print(quantized_t.shape, quantized_b.shape)
        #upsample_t = self.upsample_t(quantized_t)
        upsample_t = self.upsample_tx2(quantized_t)
        upsample_m = self.upsample_t(quantized_m)
        quantized = torch.cat([upsample_t, upsample_m, quantized_b], 1)
        dec = self.decoder(quantized).reshape(-1, 4, 128*80) # 4, 256*80
        dec = self.convTrans1d(dec).reshape(-1, 1, 80, 512) # 80, 1024
    ...
    def forward(self, x):
        quantized, loss, preplexity, ids = self.encode(x)
        recon = self.decode(quantized[0], quantized[1], quantized[2])
